...
const ExerciseCard = ({ exercise, i }) => {
  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      ...
      <div className="flex flex-col bg-slate-950 gap-2 rounded p-2">
        {exercise.description.split("___").map((val,i) => {
          return <div key={i} className="text-sm">{val}</div>;
        })}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info) => {
          return (
            <div
              key={info}
              className="flex flex-col rounded border-[1.5px] border-solid border-slate-900 w-full p-2"
            >
        ...
        <button
          ...
          className="flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-blue-900 hover:border-blue-600 w-full items-center"
        >
